...
def craetereadme(repoName, dataModel, globalUser, token):
    ...
    readmeText = "[![Smart Data Models](https://smartdatamodels.org/wp-content/uploads/2022/01/SmartDataModels_logo.png \"Logo\")](https://smartdatamodels.org)" + chr(10)  + chr(13)
    ...
    if "swagger.yaml" in files:
        ...
        readmeText += chr(10) + "Link to the [interactive specification](https://swagger.lab.fiware.org/?url=" + urlSwagger + ")" + chr(10)
    ...
    if "examples/example.json.csv" in files:
        ...
        readmeText += chr(10) + "Link to the [example](" + urlExampleKV2CSV + ") (keyvalues) for NGSI v2 in CSV format" + chr(10)
    if "examples/example.jsonld.csv" in files:
        urlExampleKVLDCSV = urlSpecBase + "/examples/example.jsonld.csv"
        readmeText += chr(10) + "Link to the [example](" + urlExampleKVLDCSV + ") (keyvalues) for NGSI-LD in CSV format" + chr(10)
    if "examples/example-normalized.json.csv" in files:
        urlExampleNV2CSV = urlSpecBase + "/examples/example-normalized.json.csv"
        readmeText += chr(10) + "Link to the [example](" + urlExampleNV2CSV + ") (normalized) for NGSI-V2 in CSV format" + chr(10)
    if "examples/example-normalized.jsonld.csv" in files:
        urlExampleNLDCSV = urlSpecBase + "/examples/example-normalized.jsonld.csv"
        readmeText += chr(10) + "Link to the [example](" + urlExampleNLDCSV + ") (normalized) for NGSI-LD in CSV format" + chr(10)
    ...
    if "schema.json" in files:
        ...
        readmeText += chr(10) + "Link to the [Generator](" + urlNGSILDExample + ") of NGSI-LD normalized payloads compliant with this data model. Refresh for new values" + chr(10)
        readmeText += chr(10) + "Link to the [Generator](" + urlNGSILDExampleKeyvalues + ") of NGSI-LD keyvalues payloads compliant with this data model. Refresh for new values" + chr(10)
        readmeText += chr(10) + "Link to the [Generator](" + urlGeojsonFeatures + ") of geojson feature format payloads compliant with this data model. Refresh for new values" + chr(10)
    ...
    readmeText += chr(10) + " If you have any issue on this data model you can raise an [issue](" + urlIssue + ")  or contribute with a [PR](" + urlPull + ")" + chr(10)

    readmeText += chr(10) + " If you wish to develop your own data model you can start from [contribution manual](" + urlContributionManual + "). Several services have been developed to help with: "
    readmeText += chr(10) + " - [Test data model repository](" + urlTest + ") including the schema and example payloads, etc"
    readmeText += chr(10) + " - [Generate PostgreSQL schema](" + urlSQLGen +") to help create a table, create type, etc"
    return readmeText
