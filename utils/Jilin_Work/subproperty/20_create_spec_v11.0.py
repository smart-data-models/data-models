...
def translation(sentence, sourceLang, targetLang, authKey):
    ...
    supportedTargetLanguages = ["DE", "EN-GB", "EN-US", "ES", "FR", "IT", "JA", "NL", "PL", "PT-PT", "PT-BR", "RU", "ZH", "KO"]
    encodings = {}
    # encodings["DE"] = "cp1252"
    encodings["DE"] = "utf-8"
        ...
        for lang in languages:
            ...
            if request.status_code == 200:
                ...
                def list_all_properties(schema, level):
                    """
                    List all properties in the recursion in a tree structure

                    Parameters:
                    - schema (dict): the schema to be parsed
                    - level (int): the level of recursion

                    Returns:
                    - (str): a string of ordered properties list 
                    """
                    if level >= 3: return ""
                    tmpSpecContent = ""
                    for prop in schema["properties"]:
                        ...
                        try:
                            ...
                            # obtain the description of property
                            ...
                            # if exists subproperties, then add it after the properties in a tree structure
                            if partialoutput:
                                tmpSpecContent += "\t" + chr(13).join(partialoutput.split(chr(13))[:-1]).replace(chr(13), "\n"+"\t"*level) + "\n" + partialoutput.split(chr(13))[-1]
            ...
            #########################################
            specContent += comment_spec("40-NotesYaml", True)
            ...
            try:
                ...
                nada = 0
            specContent += comment_spec("40-NotesYaml", False)
            #########################################
            # 5: Data model header                  #
            #########################################
