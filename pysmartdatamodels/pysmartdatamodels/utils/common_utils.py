...
def is_url_existed(url: str) -> list[bool, str]:
    """
...
#########################################
#   Convertion between different examples 
#########################################
    ...
    for element in normalizedDict:
        try:
            ...
            if "value" in normalizedDict[element]:
                ...
                # DateTime as a sub-structure inside of `value`
        ...
        # If the body is a dictionary, then process it in detail
            ...
            if "@value" in body:
                ...
                # then recursively process the list, get the sub-structure of `value` if it exists
                    ...
                    output[element] = [normalized2keyvalues_v2(subvalue, level + 1).get("value", subvalue) for subvalue in value]
            ...
            if detailed:
                if len(keyvaluesDict[element]) > 0 and isinstance(keyvaluesDict[element][0], dict):
                    ...
                    for idx in range(len(keyvaluesDict[element])):
                        tmpList.append(keyvalues2normalized_ngsild(keyvaluesDict[element][idx], yamlDict[element][idx], level=level+1))
                    item["value"] = tmpList
                else:
...
def payload_relationship(propertyName, dataModel):
    ...
    """
    return {"type": "Relationship", "object": fake_uri(propertyName, dataModel)}
...
def parse_schema_description(description: str) -> tuple[dict, str]:
    """
...
def parse_property2ngsild_example(fullPayload: dict, dataModel: str, level: int=0) -> dict:
    ...
    if prop == "id" and level == 0:
        ...
        if "required" in fullPayload[prop]:
            del fullPayload[prop]["required"]
        return parse_property2ngsild_example({propertyName: fullPayload[prop]["patternProperties"][keyPatterned]}, dataModel, level+1)
...
def parse_payload_v2(schemaPayload, level: int=1) -> tuple[dict, dict]:
    ...
    def process_allOf_oneOf_anyOf(ofSubSchema):
        ...
        for index in range(len(ofSubSchema)):
            ...
            output = dict(output, **partialOutput["properties"] if "properties" in ofSubSchema[index] else partialOutput)
    ...
    if level == 1: # first level starts always with "allOf", "properties", "anyOf", "oneOf"
    ...
    elif level < 8:  # level from 2 to 8 will be processed as below, in case of the infinit loop or too massive schema
        ...
        # if schema is a dictionay, then go inside directly
        # Example: {"allOf": [{...}, ...], "properties": {...}, "description": "xxxx", ...}
        if isinstance(schemaPayload, dict):

            for subschema in schemaPayload: # run through the each key
                ...
                if subschema in ["allOf", "anyOf", "oneOf"]:
                    ...
                    for index in range(len(schemaPayload[subschema])):
                        ...
                        output[subschema].append(partialOutput["properties"] if "properties" in schemaPayload[subschema][index] else partialOutput)
                        attributes = merge_duplicate_attributes(attributes, partialAttr)
                    ...
                    for prop in schemaPayload["properties"]:
                        ...
                        for item in list(schemaPayload["properties"][prop]):
                            ...
                            # change the type integer to number
                                ...
                                output[subschema][prop][item] = "number" if schemaPayload[subschema][prop][item] == "integer" else schemaPayload[subschema][prop][item]
...
def is_metadata_existed(output: dict, jsonDict: dict, schemaUrl: str, message: str="", checkall: bool=True, checklist: list=None) -> dict:
    """
            ...
            if field in jsonDict:
                ...
                if "value" in constraints:
                    if value != constraints:
                        if field == "$schema":
                            ...
                            output["metadata"][field] = {"warning": f"{field} doesn't match, please check it again or ignore this message if it's an unpublished datamodel"}
                ...
                if "mandatory_fields" in constraints:
                    for mf in constraints["mandatory_fields"]:
                        if mf not in value:
                            output["metadata"][field] = {"warning": f"{field} should include {', '.join(constraints['mandatory_fields'])}"}
            else:
                output["metadata"][field] = {"warning": f"Missing {field} clause, include {field} = '' in the header"}
        except Exception as e:
            output["metadata"][field] = {"warning": f"not possible to check {field} clause, {str(e)}"}
...
def schema_output_sum(output: dict) -> dict:
    ...
    for prop, value in documentationStatusOfProperties.items():
        ...
        if (prop == "type") and (value["type_specific"] is False):
            # If the property is `type`, then it has to been the same as datamodel
